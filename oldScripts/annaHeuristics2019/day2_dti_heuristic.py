...
t1w_d = create_key('sub-{subject}/{session}/anat/sub-{subject}_{session}_acq-memprage_T1w')
...
def infotodict(seqinfo):
    ...
    info = {t1w: [], t1w_d:[], pcasl: [], face_01: [], face_02: [], card_01: [], card_02: [],
            b0_mag: [], b0_phase: [], rest: [], pcasl_moco: []
            }
    for s in seqinfo:
        protocol = s.protocol_name.lower()
        # t1
        if "T1_MEMPRAGE_RMS" in s.series_description:
            info[t1w].append(s.series_id)
        elif "mprage" in protocol:
            info[t1w_d].append(s.series_id)
